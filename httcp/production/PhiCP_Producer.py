...
def produce_phi_cp_dev(
    ...
    mask_p4_dict = lambda p4dict, mask : {key: val[mask] for key, val in p4dict.items()}
    
    is_mu     = lambda leg: ak.fill_none(ak.firsts(leg.decayMode == -2, axis=1), False)
    is_pi     = lambda leg: ak.fill_none(ak.firsts(leg.decayMode ==  0, axis=1), False)

    p4h1      = p4_hcand_info["p4h1"]
    p4h2      = p4_hcand_info["p4h2"]
    ...
    empty_phi_cp = ak.values_astype(events.event[:,None][:,:0], np.float32)
    phi_cp_ipip = empty_phi_cp
    phi_cp_ipip[mask_mu_pi] = GetPhiCP(mask_p4_dict(p4_hcand_info, mask_mu_pi), "IP", "IP", "mu", "pi")
    events = set_ak_column(events, "phi_cp_ipip", phi_cp_ipip)
