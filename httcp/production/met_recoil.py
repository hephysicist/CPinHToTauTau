...
def gen_boson(
    ...
    gen_cuts = ((abs(gen_part.pdgId) >= 11) & (abs(gen_part.pdgId) <= 16) & (gen_part.statusFlags & 256 != 0) & (gen_part.status == 1)) | (gen_part.statusFlags & 1024 != 0)

    # gen neutrinos from tau decays
    nus_gen_cuts = ((abs(gen_part.pdgId) == 12) | (abs(gen_part.pdgId) == 14) | (abs(gen_part.pdgId) == 16)) & (gen_part.status == 1) & (gen_part.statusFlags & 1024 != 0)

    # get visible decay products (pdgId 11, 13, 15)
    gen_vis_cuts = gen_cuts & ~nus_gen_cuts

    # top quarks for top pt reweighting
    gen_top_cuts = ((abs(gen_part.pdgId) == 6) & (gen_part.statusFlags & 256 != 0) & (gen_part.statusFlags & 8192 != 0))
    ...
    gen_obj = get_lep_p4(gen_part[gen_cuts]).sum()
    vis_obj = get_lep_p4(gen_part[nus_gen_cuts]).sum()
...
@producer(
    uses={
        'PuppiMET*', 'n_jets_recoil', 'event', optional('gen_boson*')
    },
    produces={
        'PuppiMET*'
    },
...
def met_recoil(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    if dataset in met_recoil_dict.keys():
        ...
        boson_p4     = events.gen_boson
        boson_p4_vis = events.gen_boson_vis
        ...
        njets      = flat_np_view(events.n_jets_recoil).astype(np.float32)
