...
def higgscand(
    ...
    hcand_array = enforce_hcand_type(hcand_pair_concat, var_dict)
...
def higgscandprod(
    ...
    if ak.any(np.isnan(hcand_concat.IPx)) :
        hcand_concat = ak.where(ak.any(np.isnan(hcand_concat.IPx),axis = 1),hcand_concat, hcand_concat[..., :0])
