...
@calibrator(
    uses={f"Tau.{var}" for var in [
                "pt","eta","phi","mass", "decayModePNet", "genPartFlav"
                ] 
    },
    produces={
...
def tau_energy_scale(self: Calibrator, events: ak.Array, do_syst=False, **kwargs) -> ak.Array:
    ...
    tes_args = lambda events, mask, deep_tau_obj, syst: (pt[mask],
                                                        ...
                                                        syst)
    tes_nom[mask] = self.tes_corrector.evaluate(*tes_args(events, mask,deep_tau, syst))
    ...
    for var in ['pt', 'eta','phi', 'mass']:
        if var in ['pt','mass']:
            events = set_ak_column_f32(events, f"Tau.{var}_no_tes", ak.unflatten(np.asarray(getattr(tau_4vec,var)), arr_shape))
        events = set_ak_column_f32(events, f"Tau.{var}", ak.unflatten(np.asarray(getattr(corr_tau_p4,var)), arr_shape))
...
def tau_energy_scale_setup(
    ...
    **kwargs,
    ...
    correction_set = load_correction_set(bundle.files.tes)
    tagger_name = self.config_inst.x.deep_tau.tagger
    year = self.config_inst.x.year
    tag = self.config_inst.x.long_tag
    self.tes_corrector = correction_set[f"tau_es_dm_{tagger_name}_{year}_{tag}"]
